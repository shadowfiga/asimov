...
internal static class Program
{
    public static void Main(string[] args)
    {
        ...
        {
            StartupChecks.Run();
            using var game = new GraphicsChecks();
            game.Run();
        }
    ...
    private static void AnimationChecks()
    {
        ...
        var newer = first.Play(definition with
        {
            Tracks = [new UIParameterTrack<float>(parameter, 20, 30, MathHelper.Lerp)]
        });
        first.Update(.1f);
        Near(first.Parameters.Get(parameter), 21, "Newest parameter track wins");
        newer.Cancel();
        first.Update(0);
        Near(first.Parameters.Get(parameter), 3, "Previous parameter track restored");
        one.Cancel(.1f);
        first.Update(.05f);
        Near(first.Parameters.Get(parameter), 5, "Cancellation settles toward base");
        first.Update(.05f);
        Near(first.Parameters.Get(parameter), 7, "Base restored after cancellation");
        Check(one.State == UIPlaybackState.Cancelled && one.Completion.IsCompleted, "Cancellation handle completes");
        second.Update(.5f);
        Near(second.Parameters.Get(parameter), 7, "Completed tracks release parameters");
        ...
        first.Update(.1f);
        Near(first.Frame.Rotation, 5, "Rotation adds");
        Near(first.Frame.Scale.X, 6, "Scale multiplies");
        Near(first.Frame.Opacity, .25f, "Opacity multiplies");
        first.CancelAll();
        Check(first.Frame.IsIdentity, "Transforms restored");
        var loop = first.Play(definition with
        {
            Delay = .2f,
            Repeat = 0,
            Alternate = true
        ...
        first.Update(.1f);
        Near(first.Parameters.Get(parameter), 7, "Delay leaves base untouched");
        first.Update(1.6f);
        Near(first.Parameters.Get(parameter), 5, "Alternating repeat");
        loop.Cancel();
        first.Update(0);
        ...
        first.Update(.1f);
        Check(audio.Cues.Count == 0, "Delayed sound not started early");
        first.Update(.1f);
        Check(audio.Cues.Count == 1 && !sound.IsFinished, "Audio can outlive visual timeline");
        Check(audio.Cues[0].Pitch == .2f && audio.Cues[0].Pan == -.3f, "Sound parameters preserved");
        sound.Cancel();
        Check(audio.Voices.All(voice => !voice.IsPlaying), "Cancel stops owned audio");
        first.Update(0);
        var finite = first.Play(new UIAnimation { Duration = .1f });
        first.Update(.1f);
