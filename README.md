...
`Session` owns its `CurrentRun`. After preload succeeds, `SessionLoadingScene` calls `StartRun()` before publishing the session and entering `SandboxScene`. Direct gameplay entry without a run fails fast; Sandbox never creates one. `StartRun()` explicitly replaces the run with zeroed XP, biomass, kills and `DurationMs`, plus an empty modifiers array, without resetting the session's loadout or cleared sectors. Run data is part of the session save contract; gameplay does not yet update these counters.
