...
`RobotFactory` assembles a real object hierarchy: Robot → Bottom and Top → LeftWeapon/RightWeapon → Muzzle. The game-side `PlayerController` supplies movement, aim targets and trigger intent; `AimController` turns parts using world-space targets; each `WeaponComponent` owns independent fire cadence. Bullets are scene-root objects with `ProjectileComponent`, so they outlive their shooter and never follow its transform. `RobotPartRenderer` supplies the static placeholder art; no procedural walking or muzzle flash is enabled.

Every `Scene` owns `Objects` (`GameWorld`) and `Camera`. The engine automatically updates, renders and destroys its objects/components, including children. `SessionScene` only creates the robot and world presentation, reads input, and follows the player with its camera/audio listener. The reusable `GameObject`, `Component`, `Transform2D`, `SpriteRenderer` and `FrameAnimator` live in the engine; robot/combat behavior remains in `Game`. See [object/component usage and lifecycle](Engine/Objects/README.md). Sprite-sheet playback is available, but the robot keeps its static geometry until frame art is provided.
