...
Every `Scene` owns `Objects` (`GameWorld`) and `Camera`. The engine automatically updates, renders and destroys its objects/components, including children. `SandboxScene` only creates the robot and world presentation, reads input, and follows the player with its camera/audio listener. The reusable `GameObject`, `Component`, `Transform2D`, `SpriteRenderer` and `FrameAnimator` live in the engine; robot/combat behavior remains in `Game`. See [object/component usage and lifecycle](Engine/Objects/README.md). Sprite-sheet playback is available, but the robot keeps its static geometry until frame art is provided.
