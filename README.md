...
Every `Scene` owns `Objects` (`GameWorld`) and `Camera`. New roots enter through `Spawn(Prefab<T>, position, rotation)`; raw root creation is private to the engine. Prefabs receive a positioned root, build its children/components, and return a typed instance handle. The engine automatically updates, renders and destroys its objects/components, including children, and rolls back new objects if construction fails. `SandboxScene` only spawns mech and presentation prefabs, reads input, and follows the player with its camera/audio listener. The reusable `Prefab<T>`, `GameObject`, `Component`, `Transform2D`, `SpriteRenderer` and `FrameAnimator` live in the engine; mech/combat behavior remains in `Game`. See [prefab, object and component usage](Engine/Objects/README.md). Sprite-sheet playback is available, but the mech keeps its static geometry until frame art is provided.
