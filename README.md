...
`Objects.Spawn(new RobotPrefab(ChiselRobotsId.STARTER_MECH), position)` assembles a real object hierarchy: Robot → Bottom and Top → LeftWeapon/RightWeapon → Muzzle. `RobotPrefab` consumes the definition and passes only movement speed to `PlayerController`, which supplies movement, aim targets and trigger intent. `AimController` turns parts using world-space targets; each `WeaponComponent` owns independent fire cadence. Bullets use `ProjectilePrefab` to create independent scene roots, so they outlive their shooter and never follow its transform. `RobotPartRenderer` supplies the static placeholder art; no procedural walking or muzzle flash is enabled.
