...
`SandboxUI` lays out the compact Ore readout at top-right, XP and a ten-minute countdown at top-center, the named minimap at bottom-left, and pilot/HP at bottom-right. Map geometry and the pilot portrait icon are presentation stubs; HP reads the player's live health component. The XP total reads the run, but its progress bar stays empty until level thresholds exist. The countdown stops at `00:00` without ending the sandbox; it pauses when the game loses focus. There are no waves, objectives, fuel or ammo widgets. HUD dimensions come from existing theme tokens, with no new classes or per-screen resize handling. The graphics tests capture `.artifacts/ui-checks/sandbox-hud-final.png` at 1440p, including the actual screen filter; its Ore and XP amounts are test fixtures, not starting balances.

WASD or arrow keys move the mech; the legs face the movement direction. Their placeholder pose is static; frame-based animation will be added later. The torso and each arm gun aim independently at the mouse's world position, with a mirrored 15° maximum inward rotation relative to the torso. The reticle reads the mouse directly while rendering; it has no player target or scene-fed position. Close aim positions clamp the barrels and projectile directions at that limit; distant positions still converge normally. Outward aiming and torso rotation remain unrestricted. Configure `MechPrefab.MaxInwardAngle` in radians when constructing the prefab; the existing `AimController` enforces its parent-relative joint limits on attachment and each update. Hold left mouse to fire both guns. Diagonal movement is normalized. The camera follows the mech, and aiming uses world coordinates independently of the UI scale. Losing window focus pauses the simulation and requires releasing the fire button before shooting again.

`Objects.Spawn(new MechPrefab(SessionManager.ActiveSession.CurrentLoadout), position)` assembles a real object hierarchy: Player → Bottom and Top → LeftWeapon/RightWeapon → Muzzle. `MechPrefab` gives the root the stable name exposed by `MechPrefab.PlayerObjectName`, captures the loadout's chassis/pilot/left-weapon/right-weapon IDs, and reads Chisel columns directly. `PlayerController` retains the selected pilot ID and supplies movement, aim targets and trigger intent; pilots are presentation-only, not a second source of combat stats. `AimController` turns parts using world-space targets; each `WeaponComponent` owns independent fire cadence and reads weapon data directly through its Chisel weapon ID. Bullets use `ProjectilePrefab` to create independent scene roots, so they outlive their shooter and never follow its transform. `MechPartRenderer` supplies the static placeholder art; no procedural walking or muzzle flash is enabled.

`HealthComponent` supplies shared maximum/current health for the player and enemies while leaving death policy to their owners. The sandbox spawns one targetless `SWARMER`, resolves the player through `Objects.GetGameObjectByName(MechPrefab.PlayerObjectName)`, and calls `enemy.SetTarget(playerObject)` before the first update. The enemy caches the target's required components, pursues it directly, deals one authored contact hit, and is consumed on contact. Projectiles carry authored damage and retain their previous position for swept circle hits; a lethal shot destroys the enemy and awards its Chisel-authored XP. A zero-health player stops moving and firing, but reconstruction is not implemented. This is intentionally one enemy without waves, spawning logic, pathfinding, attack cooldowns, drops, pooling, or spatial partitioning.

Every `Scene` owns `Objects` (`GameWorld`). New roots enter through `Spawn(Prefab<T>, position, rotation)`; raw root creation is private to the engine. Prefabs receive a positioned root, build only their own children/components, and return a typed instance handle. Cross-root relationships are assigned by the scene after spawning: `GetGameObjectByName` requires one exact live match, then camera and enemy `SetTarget` calls connect the objects before simulation. The reticle is autonomous and reads the mouse through the active render camera. The engine automatically updates, renders and destroys its objects/components, including children, and rolls back new objects if construction fails. `SandboxScene` reads input and updates its audio listener/HUD. `PlayerCameraPrefab` creates an independent root with the engine's `CameraComponent`, following player position without inheriting mech rotation or scale. The component and scene backend own following and reactive viewport sizing, preserving 1600×900 reference framing independently of UI scale. `Scene.Camera` resolves the active camera object's projection, with no scene-owned fallback; UI-only scenes need no camera. The reusable `Prefab<T>`, `GameObject`, `Component`, `Transform2D`, `CameraComponent`, `SpriteRenderer` and `FrameAnimator` live in the engine; mech/combat behavior remains in `Game`. See [prefab, object and component usage](Engine/Objects/README.md). Sprite-sheet playback is available, but the mech keeps its static geometry until frame art is provided.

Chisel owns the input bindings and the **Chassis**, **Weapons**, **Pilot**, and **Enemies** definitions under `.chisel`. Chassis no longer select a weapon; the runtime loadout selects each arm independently. Tune the starter mech's speed, dimensions, and health or its autocannon's fire rate, projectile speed/lifetime/damage, and barrel length there. Commit the data in Chisel, then export MonoGame to this project. `GameData/Generated` is generated source, not a place to hand-edit settings. Generated `<Table>Ids` classes expose named zero-based integer constants, so columns are addressed directly, for example `ChiselChassis.MoveSpeed[ChiselChassisIds.STARTER_MECH]`; `Invalid` is `-1`. Table references and input actions use the same integer contract, with no game-side conversion extension. Loadouts still persist stable Chisel slugs rather than dense indexes. Asset and localization references retain their dedicated generated enum APIs. The committed content snapshot is included. The `Pilot` table (`.chisel/tables/user/pilots.json`) exports `ChiselPilotIds`/`ChiselPilot`; its starter row has `display_name` and an optional `UI` portrait reference. The `Enemies` table (`.chisel/tables/user/enemies.json`) exports the Swarmer's experience, health, movement speed, body radius, and contact damage. With the sibling Chisel checkout and its dependencies installed, the same exporter can also be run from this folder:
...
Normal builds use the checked-in C# exports and do not require Bun or Chisel. Mech state is currently scene-local: there is no terrain collision, weapon sound, saved mech position, or persistent health yet.
